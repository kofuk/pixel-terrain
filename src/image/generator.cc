...
namespace pixel_terrain::image {
    std::shared_ptr<std::array<image_generator::PixelState, 512 * 512>>
    image_generator::scan_chunk(anvil::chunk *chunk) const {
    ...
    void image_generator::handle_biomes(
    ...
    void image_generator::handle_inclination(
        std::shared_ptr<std::array<PixelState, 512 * 512>> pixel_states) const {
        for (int z = 0; z < 16; ++z) {
            for (int x = 1; x < 16; ++x) {
    ...
    void image_generator::process_pipeline(
        std::shared_ptr<std::array<PixelState, 512 * 512>> pixel_states) const {
        handle_biomes(pixel_states);
        handle_inclination(pixel_states);
    }

    void image_generator::generate_image(
    ...
    void image_generator::generate_chunk(anvil::chunk *chunk, int chunk_x,
                                         int chunk_z, png &image) const {
        std::shared_ptr<std::array<PixelState, 512 * 512>> pixel_states =
            scan_chunk(chunk);
    ...
    void image_generator::generate_region(
        std::shared_ptr<region_container> item) const {
