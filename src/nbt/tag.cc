...
namespace pixel_terrain::nbt {
    namespace {
        ...
        auto query_payload(nbt_path *path, tag_payload *payload)
            ...
            if (spec.category() == nbt_path::pathspec::category::INDEX) {
                if (payload->type() != tag_type::TAG_BYTE_ARRAY &&
                    payload->type() != tag_type::TAG_LIST &&
                    payload->type() != tag_type::TAG_INT_ARRAY &&
                    payload->type() != tag_type::TAG_LONG_ARRAY) {
                    return nullptr;
                }
            ...
            } else if (spec.category() == nbt_path::pathspec::category::KEY) {
        ...
        auto query_list_type(nbt_path *path, tag_payload *payload)
            ...
            return (*typed_payload)[spec.index()];
        }
    } // namespace

    auto tag::query(nbt_path *path) -> tag_payload * {
        if (path->remain()) {
            nbt_path::pathspec spec = path->get_one();
