...
namespace pixel_terrain::image {
    ...
    make_output_name_by_input(std::filesystem::path const &in_path,
    ...
    path_string format_output_name(std::string const &format, int x, int z) {
        path_string x_str = to_path_string(x), z_str = to_path_string(z),
                    result;
    ...
    std::tuple<int, int, bool> parse_region_file_path(
        ...
        if (filename.empty()) return std::make_tuple(0, 0, false);
        ...
        if (elements.size() != 4 || elements[0] != "r" || elements[3] != "mca")
